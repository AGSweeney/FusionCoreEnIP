...
typedef struct cip_lldp_data_table_object_values {
  ...
  CipOctet *mac_address;  // Points to mac_address_storage
  CipOctet mac_address_storage[6];  // Storage for MAC address
  CipShortString interface_label;
  CipOctet interface_label_storage[256];  // Storage for interface label string
  ...
  CipUdint ip_address_storage[1];  // Storage for IPv4 management address
...
EipStatus kCipLldpDataTableInit(void);